// modified version to include wideband analysis
...
// ST change START
  float wy_0, wy_1, wx;
  int w_n = 0, wyi_0 = 0, wyi_1 = 0, wyi = 0, wxi = 0, iwsel = 0, icsel = 0;
  int w_press = 0;
  //double coeff[3]
  float coeff[3];
  float w_ave = 0;
...
// Wplot selection
	...
	// Plot points
	for (i=0;i<wsel.n;i++) 
	  cpgpt1(wsel.x[i],wsel.y[i],2);
	// Plot upper bound
	/*	for (i=0;i<wsel.n;i++) {
	  if (i==0)
	    cpgmove(wsel.x[i],wsel.y[i]+wsel.w);
	  else
	    cpgdraw(wsel.x[i],wsel.y[i]+wsel.w);
	    }*/
...
// ST change END
    ...
    if (c=='h') {
      ...
      printf("r        Reset view\n");
      printf("w        Wideband point mark (appends to wide.dat)\n"); //ST add
      printf("W        Wideband screen scale reset view, leaves points alone\n"); //ST add
...
// ST change START
//Wide bandwidth signal point reduction
    if (c=='w') {
      if (w_press == 0){
	wx = x;
	wy_0 = y;
	...
	file=fopen("wide.dat","a");
	
	if (wyi_0 > wyi_1) {
	   w_n = abs(floor(wyi_0) - floor(wyi_1));
	   //double *w_array = (double *) malloc(w_n * sizeof(double));
	   float *w_array = (float *) malloc(w_n * sizeof(float));
	   //double *x_array = (double *) malloc(w_n * sizeof(double));
           float *x_array = (float *) malloc(w_n * sizeof(float));
	   for (j=1 ;j<w_n;j++) {
	     if (w_array == 0) {break;}
	     w_array[j] = log((float) s.z[wxi+s.nsub*(j+(int) wy_1)]);
	     
	     x_array[j] = (float) j;
	     //printf("1 w_array[%.10e] = %.10e\n", x_array[j], w_array[j]);
	     }
	     
	   //polynomialfit(w_n, 3, x_array, w_array, coeff);
	   quadfit(x_array, w_array, w_n, coeff);
	   //printf("a=%.10e, b=%.10e, c=%.10e\n", coeff[2], coeff[1], coeff[0]);
	   float y_peak = - (coeff[1]/ (2 * coeff[2]));
	   //double x_peak = coeff[2] * (-coeff[1]/ (2 * coeff[2]))*(-coeff[1]/ (2 * coeff[2])) + coeff[1]* (-coeff[1]/ (2 * coeff[2])) + coeff[0];
				       
	   y_peak = y_peak + wy_1;
	  
	   wf=s.freq - 0.5 * s.samp_rate + y_peak * s.samp_rate / (double) s.nchan;
	   
	   if (s.mjd[(int) wx]>1.0) {
	     fprintf(file,"%lf %lf %f %d 9999\n",s.mjd[wxi],wf,s.z[wxi+s.nsub*(int)y_peak],site_id);
	     printf("%lf %lf %f %d\n",s.mjd[wxi],wf,s.z[wxi + s.nsub*(int)y_peak],site_id);
	     csel.x[icsel]=wx;
	     csel.y[icsel]= y_peak;
	     icsel++;
	   ...
	   w_n = abs(floor(wyi_1) - floor(wyi_0));
	   float *w_array = (float *) malloc(w_n * sizeof(float));
	   float *x_array = (float *) malloc(w_n * sizeof(float));
	   for (j=1 ;j<w_n;j++) {
             w_array[j] = log((float) s.z[wxi+s.nsub*(j+(int) wy_0)]);
	     x_array[j] = (float) j;
	     //printf("1 w_array[%.10e] = %.10e\n", x_array[j], w_array[j]);
	   }
	   //polynomialfit(w_n, 3, x_array, w_array, coeff);
