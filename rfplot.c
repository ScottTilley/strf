...
double stdev(double *data, int i);
...
int main(int argc,char *argv[])
{
  ...
  int jj0,jj1;
  // NEW varibles for testing wideband stuff...
  float wy_0, wy_1, wx;
  ...
  for (;;) {
	...
	if (wyi_0 > wyi_1) {
	   ...
	   double *x_array = (double *) malloc(w_n * sizeof(double));
	   for (j=1 ;j<w_n;j++) {
	     w_array[j] = log((double) s.z[wxi+s.nsub*(j+(int) wy_0)]);
	     x_array[j] = (float) j;
